...
Map<String, String> buildHeaderTokens() {
  ...
  if (appStore.isLoggedIn) {
    print("Bearer Token: ${sharedPref.getString(TOKEN)}");
...
Future handleResponse(Response response, [bool? avoidTokenError]) async {
  ...
  if (response.statusCode == 401) {
    if (appStore.isLoggedIn) {
      throw 'Unauthenticated, please verify your OTP again.';
    } else {
...
Future<Response> buildAddVehicleHttpResponse(String endPoint,
    {Map? request}) async {
  if (await isNetworkAvailable()) {
    ...
    try {
      // إرسال طلب POST لإضافة السيارة
      Response response = await http
          .post(url, body: jsonEncode(request), headers: headers)
          .timeout(Duration(seconds: 20), onTimeout: () => throw 'Timeout');

      log('Response (POST): ${url.toString()} ${response.statusCode} ${response.body}');
